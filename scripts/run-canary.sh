...
read_profile() {
  ...
  (
    ...
    printf '%s\0' "$BIND_ADDRESS" "$PORT" "$CONTAINER_NAME" "$SERVED_MODEL_NAME" "$MAX_MODEL_LEN" "$USE_REPLAYSSM"
  ) >"$output_file"
}
...
read_profile "$CANDIDATE_ENV" "$candidate_profile"
read_profile "$CONTROL_ENV" "$control_profile"
...
[[ "${candidate[5]}" == 0 && "${control[5]}" == 0 ]] || { echo 'ReplaySSM must remain disabled' >&2; exit 2; }
...
cleanup() {
  ...
  "$DOCKER_BIN" rm -f "${candidate[2]}" >/dev/null 2>&1 || true
  ...
  if (( service_stopped == 1 )); then
    ...
    "$WAIT_SCRIPT" --base-url "http://${control[0]}:${control[1]}" --model "${control[3]}" --context "${control[4]}" --timeout 900 --container "${control[2]}"
  fi
  rm -rf "$tmp_dir"
  exit "$status"
}
